...
Orthorectification_Optimized_NC <- function(
  ...
  # Correction Factors---------------------------------------------------------
  slope_lon_relative = 1.0006068436165065,
  slope_lat_relative = 1.00069955287567,
  intercept_lon_relative = 0.060899305912698765,
  intercept_lat_relative = -0.03141543483565101,
  slope_lon_absolute = 0.9978591086443978,
  slope_lat_absolute = 0.9797493420639262,
  intercept_lon_absolute = -0.22226945526247732,
  intercept_lat_absolute = 0.6530567545311179,

 # Customize Inputs-----------------------------------------------------------

  # Netcdf flight data file
  file_flightnc = '/n/holylfs04/LABS/wofsy_lab/Lab/econway/DATA/MethaneAIR21rf05s.nc',

  # Digital Elevation Map created using DEM_elevatr.R
  file_dem = "/n/holyscratch01/wofsy_lab/econway/RF05/O2_NATIVE/100/dem.tiff",

  # Level 1 file. Must be a NetCDF
  file_L1_CH4 = "/n/holylfs04/LABS/wofsy_lab/Lab/MethaneAIR/level1/RF05/CH4Avionics/MethaneAIR_L1B_CH4_20210803T164806_20210803T164836_20210805T104136.nc",
  L1_var_time = "GEOS_5_tau",

  # Directory to store output
  dir_output = "/n/holyscratch01/wofsy_lab/benmergui/Orthorectification_Avionics/RF05/",
  ...
  var_lat                 = "GGLAT",
  var_lon                 = "GGLON",
  var_pitch               = "PITCH_IRS3",
  var_roll                = "ROLL_IRS3",
  var_heading             = "THDG_IRS3",
  ...
  L1_time <- ymd_hms("1985-01-01 00:00:00 UTC") + 60 * 60 * ncvar_get(L1_nc_CH4, L1_var_time) %>% seconds()
  ...
  lon_avionics <- ncvar_get(L1_nc_CH4, "Longitude")
  lat_avionics <- ncvar_get(L1_nc_CH4, "Latitude")
  lon_avionics_c <- ncvar_get(L1_nc_CH4, "CornerLongitude")
  lat_avionics_c <- ncvar_get(L1_nc_CH4, "CornerLatitude")

  # Register CH4
  lon_registered_CH4 <- (((lon_avionics- intercept_lon_relative)/slope_lon_relative) -
    intercept_lon_absolute)/slope_lon_absolute
  lat_registered_CH4 <- (((lat_avionics- intercept_lat_relative)/slope_lat_relative) -
    intercept_lat_absolute)/slope_lat_absolute

  lon_ll_registered_CH4 <- (((lon_avionics_c[,,1]- intercept_lon_relative)/slope_lon_relative) -
      intercept_lon_absolute)/slope_lon_absolute
  lon_lr_registered_CH4 <- (((lon_avionics_c[,,2]- intercept_lon_relative)/slope_lon_relative) -
      intercept_lon_absolute)/slope_lon_absolute
  lon_ul_registered_CH4 <- (((lon_avionics_c[,,3]- intercept_lon_relative)/slope_lon_relative) -
      intercept_lon_absolute)/slope_lon_absolute
  lon_ur_registered_CH4 <- (((lon_avionics_c[,,4]- intercept_lon_relative)/slope_lon_relative) -
      intercept_lon_absolute)/slope_lon_absolute
  lonc_registered      <- abind(lon_ll_registered_CH4, lon_lr_registered_CH4, lon_ul_registered_CH4, lon_ur_registered_CH4, along = 3)

  lat_ll_registered_CH4 <- (((lat_avionics_c[,,1]- intercept_lat_relative)/slope_lat_relative) -
      intercept_lat_absolute)/slope_lat_absolute
  lat_lr_registered_CH4 <- (((lat_avionics_c[,,2]- intercept_lat_relative)/slope_lat_relative) -
      intercept_lat_absolute)/slope_lat_absolute
  lat_ul_registered_CH4 <- (((lat_avionics_c[,,3]- intercept_lat_relative)/slope_lat_relative) -
      intercept_lat_absolute)/slope_lat_absolute
  lat_ur_registered_CH4 <- (((lat_avionics_c[,,4]- intercept_lat_relative)/slope_lat_relative) -
      intercept_lat_absolute)/slope_lat_absolute
  latc_registered     <- abind(lat_ll_registered_CH4, lat_lr_registered_CH4, lat_ul_registered_CH4, lat_ur_registered_CH4, along = 3)

  # Digital Elevation Map Height
  dem_height_registered <-
    matrix(nrow = nrow(lon_registered_CH4),
           ncol = ncol(lon_registered_CH4),
           data = raster::extract(dem,
           SpatialPoints(list(x = as.vector(lon_registered_CH4), y = as.vector(lat_registered_CH4))))
    )
  ...
  dists <- spDists(
              x = cbind(c(as.vector(lon)),
                        c(as.vector(lat))),
              y = cbind(c(as.vector(lon_registered_CH4)),
                        c(as.vector(lat_registered_CH4))),
  ...
  # Make dimensions
  xvals <- 1:points_x
  yvals <- ncvar_get(L1_nc_CH4, L1_var_time)
  zvals <- 1:3
  cvals <- 1:4
  nx <- length(xvals)
  ny <- length(yvals)
  ...
  ncvar_put(ncid_new, var_lon_reg,         lon_registered_CH4[,1:ny],    start=c(1,1),   count=c(nx,ny))
  ncvar_put(ncid_new, var_lat_reg,         lat_registered_CH4[,1:ny],    start=c(1,1),   count=c(nx,ny))

     ...
     #   self.processing_end_time = time.time()
        if(self.calibratewavelength  == True and self.refitwavelength == False):
            ...
            if(self.whichBand == 'CH4'): 
                headerStr='MethaneAIR_L1B_CH4'
                l1FitPath = os.path.join(self.l1DataDir,headerStr+str('_RF02_const_shift.txt')) 
            else:
                headerStr='MethaneAIR_L1B_O2'
                l1FitPath = os.path.join(self.l1DataDir,headerStr+str('_RF02_const_shift.txt')) 
            
            if(self.whichBand == 'O2'):
                self.logger.info('CALIBRATING OBSERVED WAVELENGTH IN O2 CHANNEL')
                row,shift = [],[]
                for line in open(l1FitPath, 'r'):
                    values = [float(s) for s in line.split()]
                    row.append(values[0])
                    shift.append(values[1])  

                for i in range(np.int(row[0]),np.int(row[-1])):   
                    self.wavCalCoef[i,0] = self.wavCalCoef[i,0] - shift[i-np.int(row[0])]
                wvlnew = np.zeros( (self.nrow,self.ncol))
                wvlnew = np.array([np.polyval(self.wavCalCoef[i,::-1],np.arange(1,self.ncol+1)) for i in range(self.nrow)])
                self.wavelength = wvlnew
                
                
                
            else:
                self.logger.info('CALIBRATING OBSERVED WAVELENGTH IN CH4 CHANNEL')
                row,shift = [],[]
                for line in open(l1FitPath, 'r'):
                    values = [float(s) for s in line.split()]
                    row.append(values[0])
        ...
        # ELSE RECALIBRATE FOR NEW GRANULES
            ...
            self.fitspectra(Data,granuleframeDateTime) 
            ...
            if(self.whichBand == 'O2'):
                ...
                for line in open(l1FitPath, 'r'):
                    ...
                    shift.append(values[1])  
                if(len(row) >= 50):
                	shiftinterp = interpolate.interp1d(row,shift)
                	points=np.arange(np.int(row[0]),np.int(row[-1]),1,dtype=np.int16)
                	newshift=shiftinterp(points)
                
            
                	for i in range(np.int(row[0]),np.int(row[-1])):   
                	    self.wavCalCoef[i,0] = self.wavCalCoef[i,0] - newshift[i-np.int(row[0])]
                	wvlnew = np.zeros( (self.nrow,self.ncol))
                	wvlnew = np.array([np.polyval(self.wavCalCoef[i,::-1],np.arange(1,self.ncol+1)) for i in range(self.nrow)])
                	self.wavelength = wvlnew
            ...
            pass
        self.logger.info('WAVELENGTH CALIBRATING COMPLETE')
